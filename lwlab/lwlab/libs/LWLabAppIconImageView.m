...
@interface LWLabAppIconImageView ()
...
@implementation LWLabAppIconImageView
- (instancetype)init{
    if (self = [super init]) {
...
//    CGSize f3_size = [self.bataLabel sizeThatFits:CGSizeZero];
//    f3.size = f3_size;
//    f3.origin.x = (bounds.size.width - f3.size.width)/2;
...
+ (UIImage *)imageWithImage:(UIImage *)image text:(NSString *)text textColor:(UIColor *)textColor textBackgroundColor:(UIColor *)textBackgroundColor{
    LWLabAppIconImageView *view = [[LWLabAppIconImageView alloc] initWithImage:image text:text];
