...
@implementation HVUIFlowLayoutButton
- (id)initWithFrame:(CGRect)frame{
	if (self=[super initWithFrame:frame]) {
		[self __myInit_HVUIFlowLayoutButton];
	}
	return self;
}
...
- (id)initWithContentStyle:(HVUIFlowLayoutButtonContentStyle)contentStyle{
	if (self=[self init]) {
		self.contentStyle = contentStyle;
	}
	return self;
}
- (void)__myInit_HVUIFlowLayoutButton{
	...
	self.contentStyle = HVUIFlowLayoutButtonContentStyleHorizontal;
}
...
- (void)setContentStyle:(HVUIFlowLayoutButtonContentStyle)contentStyle{
	_contentStyle = contentStyle;
	switch (self.contentStyle) {
		case HVUIFlowLayoutButtonContentStyleHorizontal:
			_flowLayout.layoutDirection = HVUILayoutConstraintDirectionHorizontal;
			break;
		case HVUIFlowLayoutButtonContentStyleVertical:
...
- (void)layoutSubviews{
	...
	switch (self.contentStyle) {
		case HVUIFlowLayoutButtonContentStyleHorizontal://左图标,右文本
		{
			...
			break;
		case HVUIFlowLayoutButtonContentStyleVertical://上图标,下文本
		{
			CGSize f1_size = image.size;
			if(!hiddenImage){
				f1_size = [self imageSizeWithBounds:bounds];
