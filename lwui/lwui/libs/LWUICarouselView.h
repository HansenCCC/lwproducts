...
//  轮播列表基于collectionview实现，实现轮播思路是section设置一个值(LWUICarouselViewNumberOfSections)，初始collectionview当前偏移量为section/2,从而实现循环轮播

#import <UIKit/UIKit.h>
#import "LWUICollectionViewFlowLayout.h"

#define LWUICarouselViewNumberOfSections 100 //限制
#define LWCarouselDefualtTimeInterval 4 //默认翻页时间间隙

//定义collectionview点击block
typedef void(^LWCarouselClickBlock)(NSIndexPath *indexPath);

@protocol LWUICarouselViewDelegate <NSObject>//对轮播界面的扩展
@required
...
/**
 ...
 @param indexPath      坐标
 */
-(void)lwCollectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath;
 ...
 */
-(void)lwCollectionViewDidEndScrolling:(UIScrollView *)scrollView;

@end

@interface LWUICarouselView : UIView
...
//递用链传递
@property(nonatomic, assign) id <LWUICarouselViewDelegate> delegate;
//选中block回调 协议方法优先级高于block
@property(nonatomic, strong) LWCarouselClickBlock whenClick;
...
/**
 cell尺寸  （修改方法：实现LWUICarouselViewDelegate<collectionView:layout:sizeForItemAtIndexPath:>的代理方法 或者 设置collectionViewLayout.itemSize）
 */
