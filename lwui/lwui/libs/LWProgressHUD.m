...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"LWProgressHUD needs to be accessed on the main thread.");
...
//
@property(strong, nonatomic) UITapGestureRecognizer *addHideGesture;
@end


@interface LWProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    LWProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    LWProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (LWProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            LWProgressHUD *hud = (LWProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = LWProgressHUDAnimationFade;
    _mode = LWProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (UITapGestureRecognizer *)addHideGesture{
    if (!_addHideGesture) {
        _addHideGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hideGestureHandler:)];
        [self addGestureRecognizer:_addHideGesture];
    }
    return _addHideGesture;
}
- (void)removeHideGesture{
    UITapGestureRecognizer *tap = self.addHideGesture;
    [self removeGestureRecognizer:tap];
    self.addHideGesture = nil;
}
- (void)hideGestureHandler:(UITapGestureRecognizer *)tap{
...
- (void)animateIn:(BOOL)animatingIn withType:(LWProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == LWProgressHUDAnimationZoom) {
        type = animatingIn ? LWProgressHUDAnimationZoomIn : LWProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == LWProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == LWProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == LWProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == LWProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    LWProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<LWProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    LWMBBackgroundView *backgroundView = [[LWMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = LWProgressHUDBackgroundStyleSolidColor;
    ...
    LWMBBackgroundView *bezelView = [LWMBBackgroundView new];
    ...
    UIButton *button = [LWProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    LWProgressHUDMode mode = self.mode;
    if (mode == LWProgressHUDModeIndeterminate) {
    ...
    else if (mode == LWProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LWProgressHUDModeDeterminate || mode == LWProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
        ...
        if (mode == LWProgressHUDModeAnnularDeterminate) {
            [(MBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == LWProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LWProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[LWProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[LWProgressHUD class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBRoundProgressView appearanceWhenContainedIn:[LWProgressHUD class], nil];
#else
        appearance = [MBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[LWProgressHUD class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBBarProgressView appearanceWhenContainedIn:[LWProgressHUD class], nil];
#else
        appearance = [MBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[LWProgressHUD class]]];
#endif
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    LWMBBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(LWProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == LWProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation LWMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = LWProgressHUDBackgroundStyleBlur;
            ...
            _style = LWProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(LWProgressHUDBackgroundStyle)style {
    if (style == LWProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = LWProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    LWProgressHUDBackgroundStyle style = self.style;
    if (style == LWProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == LWProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation LWProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [LWProgressHUD allHUDsForView:view];
    for (LWProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable LWProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    LWMBBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == LWProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = LWProgressHUDBackgroundStyleSolidColor;
...
@implementation LWProgressHUDRoundedButton
